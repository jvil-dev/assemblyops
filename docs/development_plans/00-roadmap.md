...
- **Roles:** Two — **Overseer** and **Volunteer**. Captains are Volunteers with an extra permission flag.
...
- **Auth:** `firebase_auth` + `google_sign_in` Flutter packages on the client; Firebase Admin SDK in SpringBoot for ID-token verification per request. **Stateless** — no SpringBoot session.
- **First overseer bootstrap:** SpringBoot reads `BOOTSTRAP_OVERSEER_EMAILS` env var; matching email's first login creates the row.
- **Volunteer invite:** pre-create user row keyed by email; row's `auth_uid` is null until first Google sign-in claims it.
- **Email domain restriction:** none (any Google account).
...
### Sprint 1.3 — Auth + role detection (Flutter + Riverpod + go_router)

**Objective:** wire up Google sign-in, role-based routing, and the unauthorized/pending-invite states.

**Steps (Flutter side, applies to `app/` and `admin/`)**

1. Add deps to `app/pubspec.yaml`: `firebase_core`, `firebase_auth`, `google_sign_in`, `go_router`, `flutter_riverpod`, `riverpod_annotation`. Dev deps: `riverpod_generator`, `build_runner`.
2. `app/lib/auth/firebase_options.dart` — generated by `flutterfire configure`.
3. `app/lib/auth/auth_provider.dart` — Riverpod `authStateProvider` exposing `User?`.
4. `app/lib/auth/role_provider.dart` — async Riverpod provider that fetches `me { id email role captainFlag }` from GraphQL once a `User` exists.
5. `app/lib/auth/login_screen.dart` — single button "Sign in with Google".
6. `app/lib/auth/pending_invite_screen.dart` — shown when `me` query returns "user not found".
7. `app/lib/router.dart` — go_router with redirect logic:
   - signed-out → `/login`
   - signed-in but unrecognized → `/pending-invite`
   - signed-in Overseer → `/overseer/attendants`
   - signed-in Volunteer → `/volunteer`
8. Mirror in `admin/` — same auth, but only Overseers (gated additionally by email allowlist).

**Steps (backend side — preview that lands fully in Sprint 1.4)**

- `users` table schema designed: `id BIGSERIAL`, `email VARCHAR UNIQUE`, `auth_uid VARCHAR NULL`, `role ENUM('OVERSEER','VOLUNTEER')`, `captain_flag BOOLEAN DEFAULT FALSE`, `created_at TIMESTAMPTZ`, `claimed_at TIMESTAMPTZ NULL`.
- Bootstrap mechanism: `BOOTSTRAP_OVERSEER_EMAILS` env var → Spring `ApplicationRunner` ensures rows exist.
- Claim logic: when a Firebase-authenticated request arrives whose `users.email` row has null `auth_uid`, fill it in.

**Files to create**

- `app/lib/auth/{firebase_options.dart,auth_provider.dart,role_provider.dart,login_screen.dart,pending_invite_screen.dart}`
- `app/lib/router.dart`
- `admin/lib/auth/...` (mirror)
- `docs/architecture/03-auth-and-roles.md`

**Acceptance criteria**

- [ ] Google sign-in works on Flutter Web at `app.assemblyops.org`
- [ ] After sign-in, an unrecognized email lands on `/pending-invite`
- [ ] An email in `BOOTSTRAP_OVERSEER_EMAILS` becomes Overseer on first sign-in
- [ ] An email pre-created as Volunteer claims its row on first sign-in
- [ ] Role guards prevent volunteers from reaching `/overseer/*` and vice versa
- [ ] `admin.assemblyops.org` only allows your email
